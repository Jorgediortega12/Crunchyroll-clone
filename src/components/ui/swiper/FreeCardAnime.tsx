...
import { FreeAnimes } from '@/seed/seed';

export const FreeCardAnime = () => {
  return (
    <>
      <Swiper
        ...
        virtual
      >
        {FreeAnimes.map((slide, index) => (
          <SwiperSlide key={index} virtualIndex={index}>
            <div className='relative group cursor-pointer'>
              <img className="w-full h-auto object-cover rounded-lg" src={slide.image} alt={`Imagen de ${slide.subtitle}`} />
              <div className="mt-3 text-white text-sm">{slide.title}</div>
              <p className="text-[#656565] text-sm mt-2">{slide.subtitle}</p>

              {/* Hover */}
              <div className='absolute inset-0 text-[13px] bg-black bg-opacity-95 opacity-0 group-hover:opacity-100 transition-opacity duration-300'>
